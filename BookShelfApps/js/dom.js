...
function addTodo() {
    ...
    const todoObject = composeTodoObject(textTodo, textDescription, timestamp, false)
...
function refreshDataFromTodos() {
    ...
    for(todo of todos){
        const newTodo = makeTodo(todo.task, todo.description, todo.timestamp, todo.isCompleted);
        newTodo[TODO_ITEMID] = todo.id;
